...
var (
	retryCount      = 5
	waitConfirmTime = 10 * time.Second
)
...
func (S *Service) publish(channelId int, ch *amqp.Channel, exchangeName string, routeKey string, data string, replyTo string, headers map[string]interface{}) (err error) {
	...
	if err != nil {
		if strings.Index(err.Error(), "channel/connection is not open") >= 0 {
			err = S.rePublish(channelId, exchangeName, err, routeKey, data, replyTo, headers)
		}
	}
	return
}

func (S *Service) rePublish(channelId int, exchangeName string, errmsg error, routeKey string, data string, replyTo string, headers map[string]interface{}) (err error) {
...
func (S *Service) PutIntoQueue(exchangeName string, routeKey string, notice string, replyTo string, headers map[string]interface{}) (message interface{}, puberr error) {
	defer func() {
		msg := recover()
		if msg != nil {
			//fmt.Println("msg: ",msg)
	...
	data := notice //S.dataForm(notice)
	var tryTime = 1

	for {
		puberr = S.publish(channelId, ch, exchangeName, routeKey, data, replyTo, headers)
		if puberr != nil {
...
func (S *Service) SetWaitConfirmTime(t time.Duration) {
	waitConfirmTime = t
}
